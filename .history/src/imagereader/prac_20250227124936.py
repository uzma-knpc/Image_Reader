...
class practice:
    ...
    def diagnose_image(self, img):
        """Diagnose image based on kurtosis, intensity and std deviation"""
        mean_intensity = np.mean(img)
        std_intensity = np.std(img)
        flat_img = img.flatten()
        kurtosis = stats.kurtosis(flat_img)
        skewness = stats.skew(flat_img)
        ...
        if kurtosis > 2.0 and mean_intensity > 0.5 and std_intensity > 0.2:
            return "High uptake regions detected with significant peak distribution. Findings suggest hyperfunctioning areas (hot spots), potentially linked to hyperthyroidism, bone metastases, or renal obstruction. Further evaluation is recommended.\n\n"
        elif kurtosis < -1.0 and mean_intensity < 0.3 and std_intensity < 0.15:
            return "Low uptake regions detected with flat distribution. Findings suggest hypo-functioning areas (cold spots), which may indicate nodules, renal dysfunction, or bone metastases. Correlation with clinical data is advised.\n\n"
        else:
            return "Normal uptake distribution pattern observed. No significant abnormalities detected in the intensity distribution.\n\n"
    ...
    def generate_report(self, scan_id, scan_name, diagnosis, metrics, doctor_name):
        """Generate medical report using diagnosis metrics"""
        report = f"""
...
- Kurtosis: {self.diagnosis_metrics["Kurtosis"]:.4f}
- Skewness: {self.diagnosis_metrics["Skewness"]:.4f}

-------------------------------------------------
***Standard Values***
- Normal: -2 < Kurtosis < 2, 0.3 < Mean Intensity < 0.5
- Hot spots: Kurtosis > 2.0, Mean Intensity > 0.5
- Cold spots: Kurtosis < -1.0, Mean Intensity < 0.3
