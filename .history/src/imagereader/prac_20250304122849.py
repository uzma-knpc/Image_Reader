...
"""
    ...
    def analyze_scan_features(self, scan_type, features, img):
        """Generate comprehensive medical report with standard sections"""
        try:
            # Calculate base metrics
            feature_mean = np.mean(features)
            feature_std = np.std(features)
...
📋 PROCEDURE
-----------
• Study Type: Thyroid Scan with Uptake
• Radiopharmaceutical: {patient_details.get('isotope', 'Tc-99m Pertechnetate')}
• Administered Dose: {patient_details.get('dose', '5-10 mCi')}
• Imaging Protocol: Static anterior images at 20 minutes post injection
• Equipment: Dual-head gamma camera with LEHR collimator
...
🔍 FINDINGS
----------
1. Thyroid Size and Position:
   • Gland Size: {'Enlarged' if feature_mean > 0.6 else 'Small' if feature_mean < 0.3 else 'Normal'}
   • Position: {'Normal anatomical position' if abs(feature_skewness) < 0.5 else 'Asymmetric positioning'}

2. Uptake Pattern:
   • Distribution: {'Heterogeneous' if feature_std > 0.25 else 'Homogeneous'}
   • Focal Areas: {
    'Multiple hot nodules present' if feature_kurtosis > 2.0
    else 'Single hot nodule present' if feature_kurtosis > 1.5
    else 'No significant focal abnormalities'
   }

3. Functional Assessment:
   • Overall Activity: {'Increased' if feature_mean > 0.6 else 'Decreased' if feature_mean < 0.3 else 'Normal'}
   • Symmetry: {'Symmetric' if abs(feature_skewness) < 0.5 else 'Asymmetric with ' + ('right' if feature_skewness > 0 else 'left') + ' predominance'}
...
--------
1. {
    'Thyroid function tests (TSH, FT4, FT3) recommended' if feature_mean > 0.6 or feature_mean < 0.3
    else 'Routine follow-up as clinically indicated'
}

2. {
    'Correlation with thyroid ultrasound advised' if feature_kurtosis > 1.5 or feature_std > 0.25
    else 'No immediate imaging follow-up required'
}

3. {
    'Nuclear Medicine consultation for radioiodine therapy planning' if feature_mean > 0.6 and feature_kurtosis > 2.0
    else 'Endocrine consultation may be considered' if feature_mean != 0.4
    else 'Regular clinical follow-up'
}

4. Follow-up scan in {
    '1-2 months' if feature_mean > 0.6
    else '3-6 months' if feature_mean < 0.3
    else '12 months if clinically indicated'
}
...
"""
            ...
            # Add similar structured reports for other scan types...
