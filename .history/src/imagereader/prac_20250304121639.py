...
"""
    ...
    def analyze_scan_features(self, scan_type, features, img):
        ...
        try:
            ...
            # Store normalized image for patient details extraction
...
"""
            if scan_type == "THYROID":
                ...
                report += f"""
🔍 SCAN TYPE: THYROID SCAN
------------------------
1. Overall Thyroid Status:
   • Function: {'Hyperactive' if feature_mean > 0.6 else 'Hypoactive' if feature_mean < 0.3 else 'Normal'}
   • Pattern: {'Heterogeneous' if feature_std > 0.25 else 'Homogeneous'}
   • Nodularity: {'Multiple nodules' if feature_kurtosis > 2.0 else 'Single nodule' if feature_kurtosis > 1.5 else 'No significant nodules'}
...
1. Uptake Pattern:
   ...
   • Focal Lesions: {'Multiple' if feature_kurtosis > 2.0 else 'Single' if feature_kurtosis > 1.5 else 'None detected'}

2. Specific Features:
   • Activity: {'Increased' if feature_mean > 0.6 else 'Decreased' if feature_mean < 0.3 else 'Normal'}
   • Symmetry: {'Asymmetric' if abs(feature_skewness) > 0.5 else 'Symmetric'}

📝 IMPRESSION
-----------
{self.generate_bone_impression(high_uptake, feature_kurtosis, feature_mean)}
"""
...
1. General Pattern:
   ...
   • Distribution: {'Heterogeneous' if feature_std > 0.25 else 'Homogeneous'}
   • Focal Areas: {'Present' if feature_kurtosis > 1.5 else 'Absent'}
   • Symmetry: {'Asymmetric' if abs(feature_skewness) > 0.5 else 'Symmetric'}

📝 IMPRESSION
-----------
Further correlation with specific scan parameters recommended.
"""
