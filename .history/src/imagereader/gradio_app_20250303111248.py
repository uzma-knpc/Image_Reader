...
def create_feature_plots(features, test_type):
    """Create feature visualization plots with test-specific thresholds"""
    ...
    ax1.set_title(f'{test_type} Feature Analysis', fontsize=14, pad=10)
    ...
    # Add test-specific thresholds
    if test_type == "THYROID":
        ax2.axvline(x=0.3, color='r', linestyle='--', label='Cold Threshold')
        ax2.axvline(x=0.7, color='g', linestyle='--', label='Hot Threshold')
    elif test_type == "BONE":
        ax2.axvline(x=0.7, color='r', linestyle='--', label='High Uptake')
    # Add thresholds for other test types...
    
    ax2.legend()

    # Statistical summary with test-specific interpretation
    stats_text = (
        f'{test_type} Statistical Analysis:\n'
...
def process_image(image, doctor_name, test_type):
    ...
    plt.axis('off')
    plt.title(f"{test_type} Scan Analysis")
    
    # Extract features and create feature plots
    tensor = obj.preprocess_image()
    plt.ioff()  # Turn off interactive mode
    features = obj.extract_features(tensor)
    fig_features = create_feature_plots(features, test_type)
    plt.ion()  # Turn on interactive mode
    
    # Generate report with test-specific analysis
    ...
    report_path = os.path.join(downloads_path, f"{test_type}_report_{doctor_name}.txt")
...
def main():
    ...
    with gr.Blocks(title="MEDICAL IMAGE ANALYSIS SYSTEM") as iface:
        ...
        with gr.Row():
            with gr.Column():
                ...
                test_type = gr.Dropdown(
                    choices=["THYROID", "BONE", "MAG3_RENAL", "DTPA", "DMSA", "HIDA", "PARATHYROID"],
        ...
        process_btn.click(
            process_image,
            inputs=[input_image, doctor_name, test_type],
