...
"""
    ...
    def generate_report(self, scan_id, scan_name, diagnosis, features, doctor_name):
        ...
        # Get analysis text safely
        analysis_text = (
            self.response_gen.text if hasattr(self, 'response_gen') and hasattr(self.response_gen, 'text')
            else str(self.response_gen) if hasattr(self, 'response_gen')
            else 'Analysis not available'
        )
        
        # Get patient details text safely
        patient_text = (
            self.response_gen1.text if hasattr(self, 'response_gen1') and hasattr(self.response_gen1, 'text')
            else str(self.response_gen1) if hasattr(self, 'response_gen1')
            else 'Patient details not available'
        )
