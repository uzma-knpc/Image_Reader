...
class practice:
    ...
    def analyze_scan_features(self, scan_type, features, img):
        ...
        try:
            ...
            self.extracted_text = self.extract_text_from_image(img)  # Store extracted text
...
👤 PATIENT DETAILS
----------------
• Name: {patient_details.get('name', 'MUHAMMAD HASNAIN')}
• Patient ID: {patient_details.get('id', '002421/25')}
• Age/Gender: {patient_details.get('age_gender', '1.1 YEARS')}
• Study: {patient_details.get('study_name', 'RENAL HEAD')}
• Pediatric State: {patient_details.get('pediatric_state', 'YES')}

💉 PROCEDURE DETAILS
-----------------
{procedure_details}
• Radiopharmaceutical: {patient_details.get('isotope', 'Tc-MAG3')}
• Diuretic Used: {patient_details.get('diuretic', 'YES')}
• Diuretic Time: {patient_details.get('diuretic_time', '15 MIN')}
...
Duty Doctor: Dr. {patient_details.get('duty_doctor', 'ON-CALL NUCLEAR PHYSICIAN')}
Validated by: Dr. {patient_details.get('referring_physician', '[PENDING VALIDATION]')}
